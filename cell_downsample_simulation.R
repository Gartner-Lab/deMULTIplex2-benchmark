...
test_text <- "simulation_model_cell_ds_"
...
set.seed(1)
use_cell_num <- round(nrow(tag_mtx) * cur_ratio)
use_cell <- sample(1:nrow(tag_mtx), use_cell_num)

tag_mtx <- tag_mtx[use_cell, ]
true_label <- true_label[use_cell]
print(length(use_cell))
print(table(true_label))
print(median(table(true_label)))

bc_demux2 <- benchmark_demultiplex2(tag_mtx, true_label,
                                    tag_mapping = tag_mapping,
                                    true.multiplet = "doublet",
                                    plot.path = plotPath, plot.name = paste0(cur_text, '_cell_ds_',cur_ratio), width = 3.5, height = 2.5,
                                    seed = 1,
                                    init.cos.cut = .5,
                                    ...
                                    plot.umap = "none",
                                    plot.diagnostics = F)
saveRDS(bc_demux2, paste0(rdsPath, paste0(cur_text, '_cell_ds_',cur_ratio), "bc_demux2.rds"))
...
bc_demux1 <- benchmark_demultiplex1(as.matrix(tag_mtx), true_label,
                                    tag_mapping = tag_mapping,
                                    true.multiplet = "doublet",
                                    plot.path = plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'deMULTIplex1_'))
...
cmds <- paste0("GMM-demux -c ./rds/tag_mtx_gmm.csv ", paste0(bcs, collapse = ","), " -x ", bcs, " -o ", paste0(cur_text, '_cell_ds_',cur_ratio), "gmmdemux_res/", bcs)
...
gmm_res <- lapply(bcs, function(bc) {
    print(bc)
    barcode.path <- paste0(savePath, paste0(cur_text, '_cell_ds_',cur_ratio),"gmmdemux_res/", bc, "/barcodes.tsv.gz")
    tryCatch({
...
bc_gmm <- confusion_stats(gmm_res_call, true_label,
                          ...
                          true.multiplet = "doublet",
                          plot.path =plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'gmmdemux_'),  width = 3.5, height = 2.5)
...
bc_seu <- benchmark_HTODemux(tag_mtx, rna_mtx = t(tag_mtx), true_label = true_label,
                             tag_mapping = tag_mapping,
                             true.multiplet = "doublet",
                             plot.path = plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'HTODemux_'), width = 3.5, height = 2.5)
...
bc_hashed = confusion_stats(hashed_call, true_label,
                            ...
                            true.multiplet = "doublet",
                            plot.path =plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'hashedDrops_'), width = 3.5, height = 2.5)

# 6. demuxmix
require(demuxmix)
#bc_mixfull <- benchmark_demuxmix_full(tag_mtx, rna_mtx, true_label, plot.path = plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'demuxmixfull_'), width = 3.5, height = 2.5)
bc_mixnaive <- benchmark_demuxmix_naive(tag_mtx, true_label = true_label,
                                        tag_mapping = tag_mapping,
                                        true.multiplet = "doublet",
                                        plot.path = plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'demuxmixnaive_'), width = 3.5, height = 2.5)
...
bc_bffraw <- confusion_stats(bff_raw_call, true_label,
                             ...
                             call.multiplet = 'Doublet',
                             true.multiplet = "doublet",
                             plot.path =plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'bffraw_'),  width = 3.5, height = 2.5)

bc_bffcluster <- confusion_stats(bff_cluster_call, true_label,
                                 tag_mapping,
                                 call.multiplet = 'Doublet',
                                 true.multiplet = "doublet",
                                 plot.path =plotPath, plot.name = paste0(paste0(cur_text, '_cell_ds_',cur_ratio), 'bffcluster_'),  width = 3.5, height = 2.5)
...
saveRDS(bc_all, paste0(rdsPath, paste0(cur_text, '_cell_ds_',cur_ratio), "bc_all.rds"))
...
write.csv(round(t(summary_df),3), paste0(plotPath, paste0(cur_text, '_cell_ds_',cur_ratio),"summary_df.csv"))
...
ggsave(paste0(plotPath, paste0(cur_text, '_cell_ds_',cur_ratio), 'bc_all_bar.pdf'), g1, width = 8, height = 8)
...
bc_res <- lapply(show_ratios, function(cur_ratio) {
    if(cur_ratio == 1) {
        ...
        res = readRDS(paste0(rdsPath, paste0(paste0(cur_text, '_cell_ds_'),cur_ratio), "bc_all.rds"))
    }
...
ggsave(paste0(plotPath, paste0(cur_text, '_cell_ds_'), "_barv1.pdf"), g1, width = 5, height =2)
...
g1 <- ggplot(plot_df, aes(x = dataset, y = value, color = method, group = method)) +
    ...
    theme(text=element_text(size=8),
          ...
          legend.margin=margin(0,0,0,0),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, paste0(cur_text, '_cell_ds_'), "_linev1.pdf"), g1, width = 4, height =1.8)
...
pdf(paste0(plotPath, paste0(cur_text, '_cell_ds_'), "singlet_summary_mtx", ".pdf"), width = 2.8, height = 1.8)
