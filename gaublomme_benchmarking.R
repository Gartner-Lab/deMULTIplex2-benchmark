...
# gaublomme_preprocess.R is required to prepare the files for this script.
...
test_text <- "benchmark_Gaublomme_"

# Data preprocessing to obtain tag_mtx, rna_mtx, true_label for benchmarking
tag_mtx <- readRDS( paste0(rdsPath, "tag_mtx.rds"))
rna_mtx <- readRDS( paste0(rdsPath, "rna_mtx.rds"))
demuxlet_res <- readRDS(paste0(rdsPath, "demuxlet_res.rds"))
...
tag_mtx <- tag_mtx[Matrix::rowSums(tag_mtx) > min.tt.umi, ]
print(dim(tag_mtx))
rna_mtx <- rna_mtx[, cells]
print(dim(rna_mtx))

true_label = demuxlet_res$assignment_simplified
names(true_label) <- demuxlet_res$BARCODE
