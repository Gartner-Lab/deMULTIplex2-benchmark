...
# Vireo result was obtained by running cellranger and vireo on the raw fastq files downloaded from GEO.


library(deMULTIplex2)
savePath <- "./data-raw/benchmark_res/Stoeckius_8donor/"; dir.create(savePath)
...
test_text <- "benchmark_Stoeckius_8donor_"

# Data preprocessing to obtain tag_mtx, rna_mtx, true_label for benchmarking
pbmc.umis <- readRDS(paste0(dataPath, "pbmc_umi_mtx.rds"))
pbmc.htos <- readRDS(paste0(dataPath, "pbmc_hto_mtx.rds"))
...
pbmc.hashtag  <- readRDS(paste0(rdsPath, "pbmc.hashtag.rds"))
vireo_res <- read.delim(paste0(savePath, "vireo_12/donor_ids.tsv"))

hto_call <- pbmc.hashtag$HTO_maxID
names(hto_call) <- colnames(pbmc.hashtag)

# Use common set of cells
vireo_res$cell <- gsub("-1", "", vireo_res$cell)
rownames(vireo_res) <- vireo_res$cell
use_cell <- intersect(vireo_res$cell, rownames(tag_mtx))
vireo_res <-vireo_res[use_cell, ]
tag_mtx <- tag_mtx[use_cell, ]
rna_mtx <- rna_mtx[,use_cell]


true_label = vireo_res$donor_id
names(true_label) <- vireo_res$cell
call_mapping <- as.data.frame.matrix(table(true_label[names(hto_call)], hto_call))
tag_mapping <- apply(call_mapping, 2, function(x) rownames(call_mapping)[which.max(x)])
names(tag_mapping) <- gsub("-", "_", names(tag_mapping))

true_label[!true_label %in% c(tag_mapping, 'doublet', 'unassigned')] = 'unassigned'

tag_mapping <- data.frame(tag =names(tag_mapping), true_label = tag_mapping)


# Save matrix as example dataset
stoeckius_pbmc <- tag_mtx
save(stoeckius_pbmc, file="data/stoeckius_pbmc.RData")
...
pdf(paste0(plotPath, test_text, "accmtx", ".pdf"), width = 5, height = 3)
...
# 4. HTODemux
tag_mtx_seu <- tag_mtx;colnames(tag_mtx_seu) <- gsub("_", ".",colnames(tag_mtx_seu))
tag_mapping_seu <- tag_mapping; tag_mapping_seu$tag <- gsub("_", ".",tag_mapping_seu$tag)
bc_seu <- benchmark_HTODemux(tag_mtx_seu, rna_mtx = rna_mtx, true_label = true_label,
                             tag_mapping = tag_mapping_seu,
                             true.multiplet = "doublet",
...
use_cells <- Reduce(intersect, list(names(true_label), rownames(tag_mtx), rownames(bc_demux2$res$assign_table)))
true_label_tag <- tag_mapping$tag[match(true_label[use_cells], tag_mapping$true_label)]
true_label_tag[is.na(true_label_tag)] = "n/d"
names(true_label_tag) <- names(true_label[use_cells])

dmmFull <- demuxmix(as.matrix(t(tag_mtx[use_cells,])), rna = Matrix::colSums(rna_mtx[,use_cells] > 0))
full_post <- t(dmmFull@posteriorProb)

dmmNaive <- demuxmix(as.matrix(t(tag_mtx[use_cells,])), model = "naive")
...
demux2_roc <- prob_to_mean_roc(bc_demux2$res$prob_mtx[use_cells,], true_label_tag)
