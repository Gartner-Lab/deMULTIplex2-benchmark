...
test_text <- "McGinnis_8donor_SCMK_benchmarking_"
...
tag_mtx <- read.csv(paste0(savePath, 'data/AH_SCMK/GSM4904939_8donor_PBMC_AH_SCMK_matrix.csv'), row.names = 1)
tag_mtx <- tag_mtx[,!colnames(tag_mtx) %in% c("nUMI", "nUMI_total")]
tag_orig_assign <- read.csv(paste0(savePath, 'data/AH_SCMK/GSM4904939_8donor_PBMC_AH_SCMK_barcodes.csv'), row.names = 1)
...
tag_mapping <- apply(tag_mapping[,1:8], 1, function(x)colnames(tag_mapping)[which.max(x)])
...
seu_rna <- readRDS("./data-raw/benchmark_res/McGinnis_8donor_MULTI/rds/McGinnis_8donor_MULTI_benchmarking_seu_rna.rds")
all(colnames(seu_rna) %in% colnames(rna_mtx))
seu_rna <- seu_rna[,colnames(rna_mtx)]
saveRDS(seu_rna, paste0(rdsPath, test_text, "seu_rna.rds"))
...
g1<- ggplot(umap_rqr, aes_string("UMAP_1", "UMAP_2")) +
    ...
    theme(text=element_text(size=8),
          ...
          #legend.box.margin=margin(-10,-10,-10,-10),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, "_umap_rqr.pdf"), g1, width = 1.8, height =1.7)
...
write10xCounts(paste0(rdsPath, "rna_mtx.h5"), rna_mtx_write, version='3')

# Run code on docker
#scCloud demuxEM -p 8 --hash-type nuclei-hashing --min-num-genes 100 --generate-diagnostic-plots tag_mtx.csv rna_mtx.h5 demuxEM_res
library(anndata)
adata <- read_h5ad(paste0(rdsPath, "demuxEM_res_demux.h5ad"))
...
resPath <- paste0(savePath, "gmmdemux_res/"); dir.create(resPath)
cmds <- paste0("GMM-demux -c ./rds/tag_mtx_gmm.csv ", paste0(bcs, collapse = ","), " -x ", bcs, " -o gmmdemux_res/", bcs)
...
gmm_res <- lapply(bcs, function(bc) {
    print(bc)
    barcode.path <- paste0(savePath, "gmmdemux_res/", bc, "/barcodes.tsv.gz")
    tryCatch({
...
# Combine all results
bc_all <- list(
    deMULTIplex2 = bc_demux2,
    deMULTIplex = bc_demux1,
    demuxEM = bc_demuxEM,
    GMM_Demux = bc_gmm,
    HTODemux = bc_seu,
    hashedDrops = bc_hashed,
    demuxmix = bc_mixfull,
    demuxmix_naive = bc_mixnaive,
    bff_raw = bc_bffraw,
    bff_cluster = bc_bffcluster
)
...
call_labels <- list(
    deMULTIplex2 = demux2_call,
    deMULTIplex = bc_demux1$res,
    demuxEM = demuxem_call,
    GMM_Demux = gmm_res_call,
    HTODemux = bc_seu$res,
    hashedDrops = hashed_call,
    demuxmix = bc_mixfull$call,
    demuxmix_naive = bc_mixnaive$call,
    bff_raw = bff_raw_call,
    bff_cluster = bff_cluster_call
)
...
g1 <- ggplot(plot_df, aes(x = Method, y = value, fill = Method)) +
    ...
    geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(1),dotsize = 1.2,stroke = 0.5)+
    stat_summary(fun.y=mean, geom="point", shape=23, size=1.7) +
    ...
    theme(text=element_text(size=8),
          ...
          #legend.box.margin=margin(-10,-10,-10,-10),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, 'bc_precision_recall_boxplot.pdf'), g1, width =2.3, height = 2.5)
...
g1<-ggplot(plot_df, aes(Method, F_score)) +
    ...
    theme(text=element_text(size=8),
          ...
          #legend.box.margin=margin(-10,-10,-10,-10),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, 'bc_f_score_bar.pdf'), g1, width = 2.3, height = 1.8)
...
# Cell type bias
seu_rna <- readRDS(paste0(rdsPath, test_text, "seu_rna.rds"))
call_labels <- readRDS(paste0(rdsPath, test_text, "call_labels.rds"))


umap_rna <- cbind(seu_rna@reductions$umap@cell.embeddings, seu_rna@meta.data)

ctype_level = c("T cells",  "NK cells", "B cells", "Myeloid cells")
use_color = get_factor_color(ctype_level, "Paired")
names(use_color) <- ctype_level
call_labels <- readRDS(paste0(rdsPath, test_text, "call_labels.rds"))
umap_rna$demux2_call <- call_labels$deMULTIplex2[rownames(umap_rna)]
umap_rna$cell_type_demux2 <- as.character(umap_rna$cell_type)
umap_rna$cell_type_demux2[umap_rna$demux2_call == 'Multiplet'] = 'doublet'
umap_rna$cell_type_demux2[umap_rna$demux2_call == 'Negative'] = 'negative'
use_color <- c(use_color, 'doublet' = '#525252', 'negative' = 'black')
umap_rna$cell_type_demux2 <- factor(umap_rna$cell_type_demux2, levels = names(use_color))

umap_rna$demux_call <- call_labels$deMULTIplex[rownames(umap_rna)]
umap_rna$cell_type_demux <- as.character(umap_rna$cell_type)
umap_rna$cell_type_demux[umap_rna$demux_call == 'Doublet'] = 'doublet'
umap_rna$cell_type_demux[umap_rna$demux_call == 'Negative'] = 'negative'
umap_rna$cell_type_demux2 <- factor(umap_rna$cell_type_demux2, levels = names(use_color))


plot_col <- "cell_type_demux2"
#plot_col <- "cell_type_demux"
g1<- ggplot(umap_rna, aes_string("UMAP_1", "UMAP_2")) +
    geom_point_rast(data = umap_rna[!umap_rna[[plot_col]] %in% c('doublet', 'negative'),], aes_string(color = plot_col), stroke = 0, size = .3, alpha = .8, raster.dpi=600) +
    geom_point_rast(data = umap_rna[umap_rna[[plot_col]] %in% c('doublet', 'negative'),], aes_string(color = plot_col), stroke = 0, size = .3, alpha = .8, raster.dpi=600) +
    scale_color_manual(values = use_color, na.value='lightgrey') +
    theme_bw() +
    guides(colour = guide_legend(override.aes = list(size=3),
    ...
    theme(text=element_text(size=8),
          ...
          #legend.box.margin=margin(-10,-10,-10,-10),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, "_umap_", plot_col, "v2.pdf"), g1, width = 2.7, height =1.7)
...
show_cells <- colnames(seu_rna)

true_label_bc <- tag_mapping$tag[match(true_label, tag_mapping$true_label)]
names(true_label_bc) <- names(true_label)
true_label_bc <- true_label_bc[show_cells]

bcs = names(table(true_label_bc))
call_df<- as.data.frame(sapply(call_labels,function(x){
    y <- as.character(x[show_cells])
    ...
    y[y == true_label_bc] = 'Correct'
    y[y %in% bcs] = 'Incorrect'
    return(y)
}))
rownames(call_df) <- show_cells


call_df_bin <- as.data.frame(call_df=="Correct")

call_df$cell_type <- factor(seu_rna$cell_type, levels = c("T cells",  "NK cells", "B cells", "Myeloid cells"))

call_df_bin$cell_type <- call_df$cell_type
library(data.table)
call_df_bin <- as.data.table(call_df_bin)

call_df_bin[, lapply(.SD, mean, na.rm=TRUE), by=cell_type ]


plot_df <- reshape2::melt(call_df, id.var = "cell_type")
...
g1 <- ggplot(plot_df, aes(x = cell_type, fill = result)) +
    ...
    theme(text=element_text(size=8),
          ...
          #legend.box.margin=margin(-10,-10,-10,-10),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, "_cell_type_bias_stats.pdf"), g1, width = 5, height =2.4)
...
bc_all <- readRDS(paste0(rdsPath, "McGinnis_8donor_SCMK_benchmarking_",  "bc_all.rds"))
...
g2 <- ggplot(plot_df) +
    ...
    theme(text=element_text(size=8),
          ...
          legend.margin=margin(0,0,0,0),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, bc, "_post1_reverse.pdf"), g2, width = 1.5, height =1.6)
...
g1 <- ggplot(roc_bind, aes(mean_fpr, mean_tpr, color = method)) +
    ...
    theme(text=element_text(size=8),
          ...
          legend.margin=margin(0,0,0,0),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, "_roc.pdf"), g1, width = 3, height =1.5)
...
call_df<- as.data.frame(sapply(call_labels,function(x){
    ...
    y[y %in% c('Doublet', 'Unclassified')] = 'Unclassified/Doublet'
    return(y)
}))
rownames(call_df) <- show_cells
...
use_color <-  c('Unclassified/Doublet' = '#999999', "Incorrect" = '#e41a1c', "Correct" = '#005A9C')
plot_df$result <- factor(plot_df$result, levels = names(use_color))
g1 <- ggplot(plot_df, aes(x = cell_type, fill = result)) +
    geom_bar(position="fill") +
    facet_wrap(~method, nrow = 2)+
    ...
    theme(text=element_text(size=8),
          ...
          #legend.box.margin=margin(-10,-10,-10,-10),
          plot.margin = unit(c(0,0.4,0,0.1), "cm"))
ggsave(paste0(plotPath, test_text, "_ctype_stats_show_v2.pdf"), g1, width = 5.5, height =2.4)
